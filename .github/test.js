...
const indexFile = isCjs ? 'index.js' : 'index.mjs'
const mod = fs.readFileSync(resolve(DIR, `node_modules/vue-demi/dist/${indexFile}`), 'utf-8')
...
;(function codeCheck() {
  ...
  if (!isCjs && !/(Vue|Vue2|isVue2)/gm.test(mod)) {
    console.log('ESM:', mod)
    failed = true
  }
})()
